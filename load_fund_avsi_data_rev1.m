function [cell_fundamental_avsi_data]=load_fund_avsi_data_rev1(app,avsi_fundamental_data,tf_import_fundatmental)

%%%%%%%%Check for the cell_all_rtca_data.mat file
cell_avsi_data_filename=strcat('cell_fundamental_avsi_data.mat');
[tf_avsi_fundamental_mat]=persistent_var_exist_with_corruption(app,cell_avsi_data_filename);

if tf_import_fundatmental==1
    tf_avsi_fundamental_mat=0;
end

if tf_avsi_fundamental_mat==0    
    tic;
    [~,sheet_name]=xlsfinfo(avsi_fundamental_data); 
    toc;
            
    tic;
    [~,~,raw1]=xlsread(avsi_fundamental_data,sheet_name{1});  %%%%%%%%Load in the first Sheet
    toc;
    ...
    raw_fundamental_data_summary=raw1(2:end,[number1_idx,uc1_idx,eut1_idx,alt_ft1_idx,cf1_idx,bp1_idx,criteria1_idx]);
    
    tic;
    [num,~,raw2]=xlsread(avsi_fundamental_data,sheet_name{2});  %%%%%%%%Load in the data
    toc;
    ...
    raw_fundamental_data_expanded=raw2(4:end,[num2_idx,uc2_idx,eut2_idx,alt_ft2_idx,cf2_idx,bp2_idx,nbp2_idx,mean_idx,cdf1_idx,cdf99_idx,ncd2_idx,tf_eng_idx]);
   [x1,y1]=size(raw_fundamental_data_expanded);
   for i=1:1:x1  %%%%%%%Need to Replace 'NaN'
       for j=1:1:y1
           if isnan(raw_fundamental_data_expanded{i,j})==1
               raw_fundamental_data_expanded{i,j}=strcat('NAN');
           end
       end
   end
   
   cell_fundamental_avsi_data=cell(2,1);
   cell_fundamental_avsi_data{1}=raw_fundamental_data_summary;
   cell_fundamental_avsi_data{2}=raw_fundamental_data_expanded;
    ...
    while(retry_save==1)
        try
            save(cell_avsi_data_filename,'cell_fundamental_avsi_data') 
            pause(0.1);
            retry_save=0;
        catch
            retry_save=1;
    ...
    while(retry_load==1)
        try
            load(cell_avsi_data_filename,'cell_fundamental_avsi_data')  
            pause(0.1);
            retry_load=0;
        catch
            retry_load=1;
