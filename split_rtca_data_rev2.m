function [cell_split_rtca_data]=split_rtca_data_rev2(app,cell_all_rtca_data,data_folder,folder1,tf_resplit_data,tf_recreate_excel_plots)
...
cell_split_rtca_data_filename=strcat('cell_split_rtca_data.mat');
...
if tf_split_rtca_mat==0
    ...
    [num_data_rows,num_col]=size(cell_all_rtca_data);
    cell_split_rtca_data=cell(num_data_rows,5);  %%%%%The split data nested cell
    array_time_diff=NaN(num_data_rows,3);
    for data_row_idx=1:1:num_data_rows
        data_row_idx/num_data_rows*100
        cell_split_rtca_data{data_row_idx,1}=cell_all_rtca_data{data_row_idx,1};
        cell_split_rtca_data{data_row_idx,2}=cell_all_rtca_data{data_row_idx,2};
        cell_split_rtca_data{data_row_idx,3}=cell_all_rtca_data{data_row_idx,3};
        cell_split_rtca_data{data_row_idx,4}=cell_all_rtca_data{data_row_idx,4};
        %cell_split_rtca_data{data_row_idx,5}=cell_all_rtca_data{data_row_idx,5}; %%%%%%%%%We don't need to pull the raw data through
        
        
        %%%%%First break into frequency
        temp_full_data=cell_all_rtca_data{data_row_idx,5};
...
% % %             %%%%%%Compare the two sets of data, rectify difference
% % %             %%%%%First cut the last row of temp_freq_split_data
% % %             dual_data_freq_split=cell_all_rtca_data{data_row_idx,5+freq_idx};
% % %             [dual_length,~]=size(dual_data_freq_split);
% % %             [raw_split_length,~]=size(temp_freq_split_data);
% % %             while (dual_length~=raw_split_length)                
% % %                 if raw_split_length>dual_length
% % %                     temp_freq_split_data(end-1,:)=[];
% % %                 else
...
% % %             array_time_diff(data_row_idx,freq_idx)=max(temp_freq_split_data(1:end,end)-dual_data_freq_split(1:end,1));
% % %             array_time_diff
% % %             max(max(array_time_diff))
            
            %%%%%%1)Excel time 2)height 3)CenterF 4)RFPwrDBM 5)RFState 6)NCD 7)Corrected Time in Secondsfreq_idx
            %%%%%1) Time, 2) Height, 3)Power
            
            
            
% % %             %%%%%%First Try to Line Up the Heights
% % %             if all(round(temp_freq_split_data(2:end-2,2),1)==round(dual_data_freq_split(1:end-3,2),1))==1
% % %                 %%%%%%%Just put the time in
% % %             else
% % %                 %clc
% % %                 %horzcat(temp_freq_split_data(1:end,end),dual_data_freq_split(1:end,1),temp_freq_split_data(1:end,end)-dual_data_freq_split(1:end,1))
% % %             
% % %                 max(horzcat(temp_freq_split_data(2:end-2,2)-dual_data_freq_split(1:end-3,2)))
% % %             
% % %                'Error:Data line up Error'
% % %                pause;
% % %             end
            ...
            if tf_recreate_excel_plots==1
                ...
                hold on;
                yyaxis left
                plot(temp_freq_split_data(:,7),temp_freq_split_data(:,2),'-r')
                ylabel('Reported Height (ft)')
                yyaxis right
                int_pwr_on_idx=find(temp_freq_split_data(:,5)==1);
                plot(temp_freq_split_data(int_pwr_on_idx,7),temp_freq_split_data(int_pwr_on_idx,4),'ob')
                ylabel('Interference Power (dBm)')
                grid on;
                xlabel('Elapsed Second (Seconds)')
                title(strcat(cell_all_rtca_data{data_row_idx,1},'---Center Frequency:',num2str(uni_freq(freq_idx)),'MHz'))
                filename1=strcat(cell_all_rtca_data{data_row_idx,1},'_',num2str(uni_freq(freq_idx)),'MHz.png');
            ...
            for j=1:1:num_uni_int_pwr
                ...
                if contains(cell_split_rtca_data{data_row_idx,3},'G')==1
                    if j==num_uni_int_pwr
                        temp_int_row_idx=last_cut_idx+1:1:length(temp_freq_split_data);
                    elseif j==num_uni_int_pwr-1
                        ...
                        first_temp_int_row_idx=1:1:ia_idx(j+1)-1;
                        first_cut=temp_freq_split_data(first_temp_int_row_idx,:);
                        new_cut_idx=find(first_cut(:,5)==1,1,'last');
                        temp_int_row_idx=last_cut_idx+1:1:new_cut_idx;
                    end
                    ...
                    temp_int_row_idx=find(temp_freq_split_data(:,4)==uni_int_pwr(j));
                end
        ...
        cell_split_rtca_data{data_row_idx,5}=expand_cell_data_freq_split;
    ...
    while(retry_save==1)
        try
            save(cell_split_rtca_data_filename,'cell_split_rtca_data')  %%%%%%%%About 25MB?
            pause(0.1);
            retry_save=0;
        catch
            retry_save=1;
    ...
    while(retry_load==1)
        try
            load(cell_split_rtca_data_filename,'cell_split_rtca_data')  %%%%%%%%About 25MB?
            pause(0.1);
            retry_load=0;
        catch
            retry_load=1;
